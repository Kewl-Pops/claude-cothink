...
## Files
- `cothink.py` — the driver (`init`, `run`, `doctor`, `runs`). `config.json` — engine map, model pins, per-role fallback chains, families, `max_iters`, `stop_when_blocked`, durable memory.
